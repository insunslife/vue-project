<template>
  <div id="findPw" class="modal-wrap">
    <div class="modal-content">
      <div class="modal-head">
        <h4>비밀번호 찾기</h4>
        <button type="button" class="btn-modal-close" @click="closeModal('PwFindModal')">
        ...
        <form>
          ...
          <ul>
            ...
            <li>
              ...
              <input id="input-code" v-model="companyCode" placeholder="기업고유코드" />
            </li>
            <li>
            ...
            <li v-if="certificationType === CertificationMethod.CELLPHONE">
              ...
              <div class="form-style02">
                <div class="input-wrap">
                  <span v-if="timeCheck == true" class="timer">{{ formatTime(countdown) }}</span>
            ...
            <li v-if="certificationType === CertificationMethod.EMAIL">
              ...
              <div class="form-style02">
                <div class="input-wrap">
                  <input id="find-email" v-model="email" type="text" placeholder="이메일주소입력" />
                </div>
          ...
          <div class="form-button-wrap">
            <button type="button" class="form-button" @click="findPassword">비밀번호 찾기</button>
          </div>
        </form>
      </div>
    </div>
    <div class="dimmed" @click="closeModal('PwFindModal')"></div>
  </div>
</template>
...
import type { B2BFindPasswordRequest, B2BFindPasswordResponse } from '@/types/auth/signIn'
...
console.log('### PwFindModal store', store.state.IdFindModal.toString())
...
async function findPassword() {
  const payload: B2BFindPasswordRequest = {
    userId: userId.value,
    userName: userName.value,
    certificationType: certificationType.value, // 인증방법 (CELLPHONE: 휴대전화, EMAIL: 이메일)
    countryCode: countryCode.value, // 국가코드
    cellPhone: cellPhone.value, // 휴대전화번호
    email: email.value, // 이메일
    companyCode: companyCode.value, // 기업 고유 코드
    pcc: pcc.value // 최종 확인 인증코드
  }

  const result: B2BFindPasswordResponse = await SignInService.findPassword(payload)
  if (result) {
    store.cpcc = result.cpcc
    openModal('PwFindChangeModal')
  }
}
</script>
